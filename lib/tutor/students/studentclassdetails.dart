  ...
  void initState() {
    ...
    loadSavedTimezone().then((_) {
      if (_selectedTimeZone != null && _selectedTimeZone.isNotEmpty) {
        context.read<StudentLessonDetailBloc>().add(FetchStudentLessonDetails(
            widget.lessonid.toString(), _selectedTimeZone));
      } else {
      ...
      body: Container(
        ...
        child: SingleChildScrollView(
          child: Padding(
            padding: EdgeInsets.all(w * 0.03),
            child:
                BlocBuilder<StudentLessonDetailBloc, StudentLessonDetailsState>(
              builder: (context, state) {
                if (state is StudentLessonDetailsLoading) {
                  return Center(
                      child: CircularProgressIndicator(
                    color: theColors.primaryColor,
                  ));
                } else if (state is StudentLessonDetailsLoaded) {
